...
from ynified.tools import gen_timestamp, gen_uuid4, join_strings, sha256_of_string
...
def test_sha256_of_string_matches_hashlib():
    assert sha256_of_string("Hello") == hashlib.sha256(b"Hello").hexdigest()


def test_sha256_of_string_coerces_non_string_input():
    assert sha256_of_string(123) == hashlib.sha256(b"123").hexdigest()
